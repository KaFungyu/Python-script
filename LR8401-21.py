# -*- coding: utf-8 -*-
"""
HIOKI LR8401-21 / LR8450 自动化测试控制台 v12.4 (硬件状态差分与渲染引擎极速优化版 - 软量程截幅保障版)
...
class HiokiPerfectApp:
    def __init__(self, root):
        ...
        for key in self.channel_keys:
            ...
            self.channel_vars[key] = {
                ...
                "lbl_ch": None,
                "cmb_range": None,
                "sync_failed": False, # 标记量程是否同步物理通道失败，触发红牌警告
    ...
    def _prepare_channel_for_range(self, ch_id, is_lr8450):
        """准备通道用于量程设定：开启通道、设电压模式、禁用 Scaling"""
        prefix_cmd = ":UNIT"
    ...
    def _set_and_verify_range(self, ch_id, range_text, is_lr8450, preferred_kinds):
        """下发量程并回读校验，preferred_kinds 为可变字典用于缓存成功的参数类型。
        返回 (success, last_readback)。"""
        prefix_cmd = ":UNIT"
        scpi_range = get_scpi_range(range_text)
        success = False
        last_readback = ""
        ...
        return success, last_readback

    def _apply_scaling_config(self, ch_id, ratio_val, is_lr8450, delay=True):
        """下发缩放比配置到指定通道"""
    ...
    def _update_selection_visuals(self):
        """更新界面组件背景高亮，反馈当前的多选状态，同步红牌显示失败的同步状态"""
        for key, vars_dict in self.channel_vars.items():
            ...
            if cmb_range:
                if sync_failed:
                    cmb_range.config(style="Red.TCombobox")
                else:
                    cmb_range.config(style="TCombobox")
        ...
        for k in keys:
            ...
            if comment:
                if success:
                    ...
                    self.channel_vars[k]["sync_failed"] = False # 恢复成功状态
                    ...
                    self.channel_vars[k]["sync_failed"] = True # 触发失败视觉标记
    ...
    def _bg_sync_all_commented_ranges_now(self, keys, is_lr8450):
        ...
        for k in keys:
            ...
            if success:
                ...
                self.channel_vars[k]["sync_failed"] = False # 同步成功，状态置信
                self.write_log(f"[连接同步成功] {ch_id} ({comment}) 的物理量程设为: {actual_range}")
            else:
                actual_range = format_range_for_log(last_readback) if last_readback else "无响应"
                self.channel_vars[k]["sync_failed"] = True # 标记为同步失败
                failed_channels.append({
                    "ch_id": ch_id,
                    "comment": comment,
                    "expected": range_text,
    ...
    def show_sync_failed_popup(self, failed_list):
        """量程物理同步/回读校验失败通道的汇总警告弹窗"""
        self.root.bell()
        
        alert_win = tk.Toplevel(self.root)
        alert_win.title("⚠️ 通道物理量程同步失败")
        alert_win.geometry("540x350")
        alert_win.configure(bg="#ffffff")
        alert_win.attributes("-topmost", True)
        ...
        try:
            ...
            alert_win.geometry(f"540x350+{x}+{y}")
        except Exception:
            pass
        ...
        main_frame = tk.Frame(
            alert_win, 
            bg="#fff5f5", # 红色浅底
            highlightbackground="#ff3b30", # 红色边框
        ...
        failed_text.config(state="disabled") # 只读
    ...
    def create_widgets(self):
        ...
        # 定义 Red.TCombobox 样式，专门用于高亮显示同步失败的物理通道
        style.configure("Red.TCombobox", 
                        fieldbackground="#ffebee", # 柔和淡粉底
                        background="#ffcdd2", 
                        foreground="#ff3b30", # 警告系统红
                        arrowcolor="#ff3b30", 
                        bordercolor="#ff8a80",
                        lightcolor="#ff8a80",
                        darkcolor="#ff8a80")
    ...
    def _update_ui_connected(self):
        ...
        self.port_combo.config(state="disabled")
        self.baudrate_combo = self.baud_combo # 避免局部引用偏差
    ...
    def show_soft_over_alert(self, channel_key, comment, raw_volt, limit_volt):
        """高度定制·防卡死非阻塞式通道软量程截幅（饱和）警报弹窗"""
        meta = self.channel_metadata[channel_key]
        ch_display = f"{meta['prefix']}_{meta['ch']}"
        
        msg = f"通道 {ch_display} ({comment}) 测量已达到或接近量程极限！\n\n当前输入电压：{raw_volt*1000:.3f} mV\n当前设定量程：{limit_volt*1000:.1f} mV\n\n信号已被截幅（饱和），MAX/MIN 统计值已失真！建议立即增大该通道的量程。"
        
        self.root.bell()
        
        alert_win = tk.Toplevel(self.root)
        alert_win.title("⚠️ 软超量程截幅告警")
        alert_win.geometry("500x230")
        alert_win.configure(bg="#ffffff")
        alert_win.attributes("-topmost", True)
        ...
        try:
            ...
            alert_win.geometry(f"500x230+{x}+{y}")
        except Exception:
            pass
        
        main_frame = tk.Frame(
            alert_win, 
            bg="#fff9db", # 黄色底色，表示警告而非严重错误
            highlightbackground="#f59e0b", # 琥珀色
        ...
        tk.Label(main_frame, text=msg, font=("Microsoft YaHei UI", 10, "bold"), fg="#1c1c1e", bg="#fff9db", justify="left", wraplength=460).pack(pady=5)
    ...
    def _bg_start_task(self):
        ...
        if getattr(self, "config_unsynced", True):
            ...
            for key in self.channel_keys:
                ...
                if comment:
                    # ==================== 活动通道（有注释） ====================
                    # 仅在量程发生改变、或者该通道原本为开启状态时，才触发量程物理指令下发和校验回读
                    ...
                    if "over" in res_lower or "nan" in res_lower or "inf" in res_lower or "o.r" in res_lower:
                        ...
                        try:
                            ...
                            if abs(val_float) >= 1.0e+9:
                                ...
                                k = chan["key"]
                                lim_info = self.channel_limits_cache[k]
                                limit_volt = lim_info["limit_volt"]
                                ratio_val = lim_info["ratio_val"]
                                
                                # 还原计算未缩放前的 raw 物理电压值
                                if lim_info["is_scaled"]:
                                    raw_volt = abs(val_float / ratio_val)
                                else:
                                    raw_volt = abs(val_float)
                                
                                # 如果还原后的物理输入电压值达到或超过了该通道名义量程的 99.9%，即视为饱和溢出
                                if raw_volt >= limit_volt * 0.999:
                                    is_over = True
                                    is_soft_over = True
                                else:
                                    is_valid = True
                ...
                if is_over:
                    ...
                    if chan["key"] not in self.alerted_over_channels:
                        ...
                        if is_soft_over:
                            ...
                            limit_volt = lim_info["limit_volt"]
                            if lim_info["is_scaled"]:
                                raw_volt = abs(val_float / lim_info["ratio_val"])
                            else:
                                raw_volt = abs(val_float)
                            msg_log = f"[量程饱和告警] 通道 {chan['scpi_ch']} ({chan['comment']}) 发生软超量程截幅！实际输入约 {raw_volt*1000:.3f}mV，已达该通道 {limit_volt*1000:.1f}mV 量程极限，统计数值已失真！请立即增大量程。"
                            self.write_log(msg_log)
                            self.root.after(0, self.show_soft_over_alert, chan["key"], chan["comment"], raw_volt, limit_volt)
                        else:
                            self.write_log(f"[硬溢出告警] 通道 {chan['scpi_ch']} ({chan['comment']}) 发生 OVER 溢出！")
